...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/currency/currencies.php");
		...
		else
		{
			...
			if (!empty($ID))
			{
				for ($i=0; $i<$iCount; $i++)
				{
					...
					$arFields['HIDE_ZERO'] = ${"HIDE_ZERO_".$arLangsLID[$i]};
			...
			LocalRedirect("/bitrix/admin/currency_edit.php?ID=".$ID."&lang=".LANGUAGE_ID);
		}
	}
	else
	{
...
if (strlen($ID) > 0 && !$bVarsFromForm)
{
	...
	while ($ar = $res->Fetch())
	{
		...
		${"l_HIDE_ZERO_".$ar["LID"]} = $ar['HIDE_ZERO'];
	}
...
if($bVarsFromForm)
{
	...
	for ($i=0; $i<$iCount; $i++)
	{
		...
		${"l_HIDE_ZERO_".$arLangsLID[$i]} = ${"HIDE_ZERO_".$arLangsLID[$i]};
	}
}
...
<?$tabControl->BeginNextTab();?>
	...
	<tr class="adm-detail-required-field">
		...
		<td width="60%">
			<input type="text" size="10" name="AMOUNT_CNT" value="<?=htmlspecialcharsbx($f_AMOUNT_CNT)?>">
		</td>
	</tr>
	...
	<tr>
		...
		<td width="60%">
			<input type="text" size="10" name="SORT" value="<?echo intval($f_SORT)?>" maxlength="10">
		</td>
	</tr>
	<?for($i=0;$i<$iCount;$i++):?>
	<tr class="heading"><td colspan="2"><?echo htmlspecialcharsex($arLangNamesLID[$i]); ?></td></tr>
	<tr>
		<td width="40%"><?echo GetMessage("CURRENCY_FULL_NAME")?>:</td>
		<td width="60%"><input title="<?echo GetMessage("CURRENCY_FULL_NAME_DESC")?>" type="text" maxlength="50" size="15" name="FULL_NAME_<?echo $arLangsLID[$i]?>" value="<?=htmlspecialcharsbx(${"l_FULL_NAME_".$arLangsLID[$i]})?>"></td>
	</tr>
	<tr>
	...
	<tr class="adm-detail-required-field">
		...
		<td width="60%"><input title="<?echo GetMessage("CURRENCY_FORMAT_DESC")?>" type="text" maxlength="50" size="10" name="FORMAT_STRING_<?echo $arLangsLID[$i]?>" value="<?=htmlspecialcharsbx(${"l_FORMAT_STRING_".$arLangsLID[$i]})?>"></td>
	</tr>
	<tr>
		<td width="40%"><?echo GetMessage("CURRENCY_DEC_POINT_DESC")?>:</td>
		<td width="60%"><input title="<?echo GetMessage("CURRENCY_DEC_POINT_DESC")?>" type="text" maxlength="5" size="5" name="DEC_POINT_<?echo $arLangsLID[$i]?>" value="<?=htmlspecialcharsbx(${"l_DEC_POINT_".$arLangsLID[$i]})?>"></td>
	</tr>
	<tr>
		...
		<td width="60%">
			...
			<input title="<?echo GetMessage("THOU_SEP_DESC")?>" type="text" maxlength="5" size="5" name="THOUSANDS_SEP_<?echo $arLangsLID[$i]?>" value="<?=htmlspecialcharsbx(${"l_THOUSANDS_SEP_".$arLangsLID[$i]})?>">
			<script type="text/javascript">
	...
	<tr>
		<td width="40%"><?echo GetMessage("DECIMALS_DESC")?>:*</td>
		<td width="60%"><input title="<?echo GetMessage("DECIMALS_DESC")?>" type="text" maxlength="5" size="5" name="DECIMALS_<?echo $arLangsLID[$i]?>" value="<?=htmlspecialcharsbx(${"l_DECIMALS_".$arLangsLID[$i]})?>"></td>
	</tr>
	<tr>
		<td width="40%"><? echo GetMessage('HIDE_ZERO_DECIMALS'); ?>:</td>
		<td width="60%">
			<input type="hidden" name="HIDE_ZERO_<? echo $arLangsLID[$i]; ?>" value="N">
			<input type="checkbox" name="HIDE_ZERO_<? echo $arLangsLID[$i]; ?>" value="Y" <? echo ('Y' == ${"l_HIDE_ZERO_".$arLangsLID[$i]} ? 'checked' : ''); ?>>
		</td>
	</tr>
	<?endfor;
$tabControl->EndTab();
...
?><br>
<?
echo GetMessage('HIDE_ZERO_DECIMALS_DESCR');
echo EndNote();
