...
{
	return '<td><input type="text" name="'.$strPrefix.$intRangeID.'_VALUE" size="13" value="'.htmlspecialcharsbx($arRange['VALUE']).'"> <select name="'.$strPrefix.$intRangeID.'_TYPE" style="width:150px;">'.
			'<option value="P" '.('P' == $arRange['TYPE'] ? 'selected' : '').'>'.htmlspecialcharsex(GetMessage('BT_CAT_DISC_SAVE_EDIT_RANGE_PERCENT')).'</option>'.
			'<option value="F" '.('F' == $arRange['TYPE'] ? 'selected' : '').'>'.htmlspecialcharsex(GetMessage('BT_CAT_DISC_SAVE_EDIT_RANGE_FIX')).'</option>'.
			'</select>'.
		'</td>';
}
