...
if (!$bReadOnly && $_SERVER['REQUEST_METHOD']=="POST" && strlen($Update)>0 && check_bitrix_sessid())
{
	...
	$arFields = array(
		"SITE_ID" => (array_key_exists('SITE_ID', $_POST) ? $_POST['SITE_ID'] : ''),
		...
		"RENEWAL" => (array_key_exists('RENEWAL', $_POST) && 'Y' == $_POST['RENEWAL'] ? 'Y' : 'N'),
		"NAME" => (array_key_exists('NAME', $_POST) ? $_POST['NAME'] : ''),
		"SORT" => (array_key_exists('SORT', $_POST) ? $_POST['SORT'] : 500),
		"MAX_DISCOUNT" => (array_key_exists('MAX_DISCOUNT', $_POST) ? $_POST['MAX_DISCOUNT'] : 0),
		"VALUE_TYPE" => (array_key_exists('VALUE_TYPE', $_POST) ? $_POST['VALUE_TYPE'] : ''),
		"VALUE" => (array_key_exists('VALUE', $_POST) ? $_POST['VALUE'] : 0),
	...
	if (!($ID > 0 && !$boolCopy))
	{
		$arCouponFields = array(
			'COUPON_ADD' => (array_key_exists('COUPON_ADD', $_POST) && 'Y' == $_POST['COUPON_ADD'] ? 'Y' : 'N'),
			'COUPON_TYPE' => (array_key_exists('COUPON_TYPE', $_POST) ? $_POST['COUPON_TYPE'] : ''),
			'COUPON_COUNT' => intval(array_key_exists('COUPON_COUNT', $_POST) ? $_POST['COUPON_COUNT'] : 0),
		);
		$boolCouponAdd = true;
