		...
		<div style="float:right">
			<script type="text/javascript">
				...
				function AddProductSearch(index)
				{
					...
					window.open('/bitrix/admin/sale_product_search.php?lang=<?=LANGUAGE_ID?>&from=order&LID=<?=CUtil::JSEscape($LID)?>&addDefault=N&caller=' + caller + '&func_name=getParamsByProductId&index=' + index + '&QUANTITY=' + quantity + '&BUYER_ID=' + BUYER_ID + '&BUYER_COUPONS=' + BUYER_COUPONS + '&ID=<?=CUtil::JSEscape($ID)?>&userColumns=' + strUserColumns, '', 'scrollbars=yes,resizable=yes,width=980,height=550,top='+parseInt((screen.height - 500)/2-14)+',left='+parseInt((screen.width - 840)/2-5));
				}
			</script>
			<?
					...
					<div class="order-itog">
						...
						<td nowrap class="sum_paid" onclick="fEditSumPaid(true);">
							...
							<span id="sum_paid_edit" style="display:none">
								<input type="text"
									size="5"
									value="<?=CurrencyFormatNumber($str_SUM_PAID, $str_CURRENCY);?>"
...
<script type="text/javascript">
	...
	function fEditSumPaid(edit)
	{
		...
		else
		{
			...
			var maxValue = parseFloat(BX('ORDER_TOTAL_PRICE').innerHTML.replace(' ', '').replace(',', '.')),
				newValue = 0,
				sumPriceValue = parseFloat(BX('ORDER_PAY_FROM_ACCOUNT_EDIT').value);

			if (!isNaN(sumPriceValue) && sumPriceValue <= maxValue)
