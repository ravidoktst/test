	...
	*/
	if (isset($type) && $type != "")
	{
		...
		if ($type == 'viewed' && CModule::includeModule("catalog"))
		{
			...
			$priceIterator = CPrice::getList(array(), array("PRODUCT_ID" => $arViewedIds, "!CURRENCY" => "",  ">PRICE" => 0,  'CATALOG_GROUP_ID' => $baseGroup['ID']), false, false, array("PRODUCT_ID", "PRICE", "CURRENCY"));
			while($productPrice = $priceIterator->fetch() )
			{
				foreach($arViewed as &$arViews)
				{
					if ($arViews["PRODUCT_ID"] == $productPrice["PRODUCT_ID"])
					{
						$arViews["PRICE"] = $productPrice["PRICE"];
						$arViews["CURRENCY"] = $productPrice["CURRENCY"];
					}
				}
			}
						...
						<tr>
							<td class="load_product" valign="top">
								...
								<div id="tabs">
									...
									$baseGroup = CCatalogGroup::getBaseGroup();
									$priceIterator = CPrice::getList(array(), array("PRODUCT_ID" => $arViewedIds, "!CURRENCY" => "",  "!PRICE" => "",  'CATALOG_GROUP_ID' => $baseGroup['ID']), false, false, array("PRODUCT_ID", "PRICE", "CURRENCY"));
									while($productPrice = $priceIterator->fetch() )
									{
										foreach($arViewed as &$arViews)
										{
